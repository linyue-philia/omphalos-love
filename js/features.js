...
function renderAlbumGrid() {
    ...
    if (count) count.textContent = (albumLibrary || []).length + ' 张';
    
    if (!albumLibrary || albumLibrary.length === 0) {
        grid.innerHTML = `
            <div style="grid-column:1/-1;text-align:center;padding:40px 10px;color:var(--text-secondary);">
                ...
                <p style="font-size:13px;">相册还是空的</p>
                <p style="font-size:11px;opacity:0.6;">点击「上传图片」添加照片</p>
            </div>
        `;
        return;
    }
    
    grid.innerHTML = albumLibrary.map((src, index) => `
        <div class="album-grid-item" style="position:relative;aspect-ratio:1/1;border-radius:10px;overflow:hidden;border:2px solid var(--border-color);background:var(--primary-bg);transition:all 0.2s;">
            <img src="${src}" style="width:100%;height:100%;object-fit:cover;cursor:pointer;" loading="lazy" onclick="if(typeof viewImage==='function')viewImage('${src.replace(/'/g,"\\'")}')">
            <button class="album-del-btn" data-index="${index}" style="position:absolute;top:4px;right:4px;width:22px;height:22px;border-radius:50%;border:none;background:rgba(0,0,0,0.55);color:#fff;font-size:11px;cursor:pointer;opacity:0;transition:opacity 0.2s;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);">✕</button>
        </div>
    `).join('');
    
    // 删除事件
    grid.querySelectorAll('.album-del-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const idx = parseInt(this.dataset.index);
            if (confirm('确定删除这张照片吗？')) {
                albumLibrary.splice(idx, 1);
                throttledSaveData();
                renderAlbumGrid();
...
function initAlbumListeners() {
    ...
    if (uploadBtn && fileInput) {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            if (oversized.length) showNotification(`${oversized.length} 张超过5MB已跳过`, 'warning');
            const valid = files.filter(f => f.size <= 5 * 1024 * 1024);
            if (!valid.length) return;
            
            showNotification(`正在处理 ${valid.length} 张...`, 'info', 1500);
            let ok = 0, fail = 0;
            for (const file of valid) {
                try {
                    const base64 = await optimizeImage(file, 600, 0.8);
                    albumLibrary.push(base64);
                    ok++;
                } catch(err) { fail++; }
            }
            throttledSaveData();
            renderAlbumGrid();
            showNotification(fail ? `已添加 ${ok} 张，${fail} 张失败` : `✓ 已添加 ${ok} 张照片`, fail ? 'warning' : 'success');
            e.target.value = '';
        });
    }
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            hideModal(document.getElementById('album-modal'));
        });
    }
}
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initAlbumListeners);
} else {
    initAlbumListeners();
}

// 暴露全局
window.renderAlbumGrid = renderAlbumGrid;
window.albumLibrary = albumLibrary;