...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
			...
			localforage.getItem(getStorageKey('albumLibrary')),
		...
		const savedAlbum = getVal(21); // 根据实际索引调整
if (savedAlbum) albumLibrary = savedAlbum;
...
const saveData = async () => {
    ...
    const promises = [
		...
		{ key: 'albumLibrary', val: () => localforage.setItem(getStorageKey('albumLibrary'), albumLibrary) },
    ];
    ...
    for (let i = 0; i < replyCount; i++) {
        ...
        setTimeout(() => {
            try {
				...
				// 相册：1% 概率发送
if (albumLibrary.length > 0 && Math.random() < 0.01) {
    setTimeout(() => {
        const img = albumLibrary[Math.floor(Math.random() * albumLibrary.length)];
        addMessage({
            id: Date.now() + Math.random(),
